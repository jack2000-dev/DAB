# Resources

## Pages

- **[Python Cheatsheet](python-cheatsheet.md)** — most-used pandas functions
- **[SQL Cheatsheet](sql-cheatsheet.md)** — most-used functions and clauses
- **[Books](books.md)** — DAMA-DMBOK, Tukey, McKinney, Wickham
- **[Tutorials & Practice](tutorials.md)** — courses, learning paths, practice platforms, portfolio & interview prep
- **[Datasets](datasets.md)** — open data sources for practice

## Courses & certifications

- [Google Data Analytics Certificate](https://www.coursera.org/professional-certificates/google-data-analytics)
- [Google Skills paths](https://www.skills.google/paths/2267)
- [Kaggle Learn](https://www.kaggle.com/learn) — short, hands-on
- [Made with ML](https://madewithml.com/) — ML learning platform
- [DataCamp](https://www.datacamp.com/) — paid; structured tracks
- [fast.ai](https://www.fast.ai/) — practical deep learning

## Communities

- [Towards Data Science](https://towardsdatascience.com/) — Medium publication
- [r/dataisbeautiful](https://www.reddit.com/r/dataisbeautiful/)
- [r/datascience](https://www.reddit.com/r/datascience/)
- [Stack Overflow](https://stackoverflow.com/questions/tagged/pandas) — pandas, sql, etc.
- [Kaggle Discussions](https://www.kaggle.com/discussions)
- [Locally Optimistic](https://locallyoptimistic.com/) — analytics engineering blog

## Newsletters

- [Data Elixir](https://dataelixir.com/) — weekly DA news
- [Benn Stancil's newsletter](https://benn.substack.com/)
- [The Data Pub](https://datapub.dev/)
- [Data Engineering Weekly](https://www.dataengineeringweekly.com/)

## Networking

- [Meetup — data analytics groups](https://www.meetup.com/topics/data-analytics/)
- [Data Science Association](https://www.datascienceassn.org/)
- [LinkedIn — DA hashtags](https://www.linkedin.com/feed/hashtag/dataanalytics/)
